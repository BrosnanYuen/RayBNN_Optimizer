...
pub fn weighted_sigmoid_cross_entropy<Z: arrayfire::FloatingPoint<AbsOutType = Z, UnaryOutType = Z>  >(
	...
	weight: f64) -> f64 {
		...
		let total = ZERO-( (weight_Z*arrayfire::mul(y, &logsigmoid, false)) + arrayfire::mul(&minus, &logminus, false)  );
		let (r0,_) = arrayfire::mean_all(&total);
		r0
}
...
pub fn weighted_sigmoid_cross_entropy_grad<Z: arrayfire::FloatingPoint<AbsOutType = Z, UnaryOutType = Z>  >(
	...
	weight: f64) -> arrayfire::Array<Z>  {
		...
		(onedivsize)*( (weight_Z*arrayfire::mul(y, &p0, false))  +    arrayfire::mul(&minus, &p1, false)    )
}
